...
- [ ] **Deploy the backend.** Today `NetworkModule.CHAT_BASE_URL` points at `http://192.168.1.200:8001` and Firebase uses local emulators, so sign-in + chat don't work for a fresh install. Deploy the FastAPI service (e.g. Cloud Run), move secrets to a secret manager, switch to a real Firebase project.
